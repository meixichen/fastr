...
test_that("Factor model in C++ has same nll as the nll computed in R. ",{
  n_test <- 10
  for (ii in 1:n_test){
    #---- Data simulation ---------
    dt <- 0.005
    ...
    data <- list(model="factor_model", n_factor=n_factor, dt=dt, Y=Y, lam=1)
...
test_that("Optimization of the factor model converges and gives PD Hessian.",{
  dt <- 0.005
  n_bin <- 2000
